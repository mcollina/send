...
var app = http.createServer(function (req, res) {
  ...
  .on('error', error)
  .on('ready', readyHandler(res, error))
})

describe('send(file).on(\'ready\')', function () {
  ...
  it('should not override content-type', function (done) {
    var app = http.createServer(function (req, res) {
      ...
      send(req, req.url, {root: fixtures}).on('ready', readyHandler(res))
    })
    request(app)
    .get('/nums')
  ...
  // TODO how would the new API handle this case?
  ...
  describe('dotfiles', function () {
    ...
    describe('when "deny"', function (done) {
      ...
      it('should 403 for dotfile without root', function (done) {
        var server = http.createServer(function onRequest (req, res) {
          send(req, fixtures + '/.mine' + req.url, {dotfiles: 'deny'})
          .on('ready', readyHandler(res))
        })
    ...
    describe('when "ignore"', function (done) {
      ...
      it('should 404 for dotfile without root', function (done) {
        var server = http.createServer(function onRequest (req, res) {
          send(req, fixtures + '/.mine' + req.url, {dotfiles: 'ignore'})
          .on('ready', readyHandler(res))
        })
...
function createServer (opts, fn) {
  return http.createServer(function onRequest (req, res) {
    try {
      fn && fn(req, res)
      send(req, req.url, opts).on('ready', readyHandler(res))
    } catch (err) {
      res.statusCode = 500
      res.end(String(err))
    }
...
function readyHandler (res, onEnd) {
  return function (statusCode, headers, res) {
    res.statusCode = statusCode
    for (var k in headers) {
      res.setHeader(k, headers[k])
    }
    pump(stream, res, onEnd)
  }
}
